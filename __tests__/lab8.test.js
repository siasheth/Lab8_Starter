describe('Basic user flow for Website', () => {
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    let cart_count_inner_text = await cart_count.getProperty('innerText');
    let cart_count_text = await cart_count_inner_text.jsonValue();
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for (i = 0; i < prodItems.length; i++) {
      ...
      // Once you have the button, you can click it and check the innerText property of the button.
      let button = await shadowRoot.$('button');
      let innTxt = await button.getProperty('innerText');
      let text = await innTxt.jsonValue();
      expect(text).toBe("Remove from Cart");
    }
